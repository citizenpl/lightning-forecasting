...
ERA_coords=[str(Lat_era_light[lat])+','+str(Lon_era_light[lon]) for lat in range(len(Lat_era_light)) for lon in range(len(Lon_era_light))]
...
input_data=input_data_help[:,:,6]
df_input_dataset = pd.DataFrame(input_data, columns=feature_array[0:-1])
df_statistics=df_input_dataset.describe(include='all')

directory_to_save_results="C:/Users/plgeo/OneDrive/PC Desktop/MATLAB DRIVE/MSc_Meteorology/regression results/"
filetosave3=directory_to_save_results+'Classifier_input_statistics.xlsx'
filetowrite3=pd.ExcelWriter(filetosave3, engine='xlsxwriter')##excel file to save processed data
df_statistics.to_excel(filetowrite3,sheet_name='Sheet1', engine="xlsxwriter", startrow=1, startcol=0, header=True)
filetowrite3.save()

# input_data_test0 = input_data_help[round(input_data_help.shape[0] * 0.75):, :,2]
# input_data_test1 = input_data_help[round(input_data_help.shape[0] * 0.75):, :, 5]
# input_data_test2 = input_data_help[round(input_data_help.shape[0] * 0.75):, :, 5]
# input_data_test3 = input_data_help[round(input_data_help.shape[0] * 0.75):, :, 6]
...
# light_test = Strike_grid_regr[round(input_data_help.shape[0] * 0.75):, index_]
# light_binary_test = Strike_grid_categ[round(input_data_help.shape[0] * 0.75):, index_]
# light_test = np.reshape(light_test,(light_test.shape[0] * 4,),order='F')
# light_binary_test = np.reshape(light_binary_test,(light_binary_test.shape[0] * 4,),order='F')
...
# print(df_test_SVR_dataset.head(5))
...
#
# Prediction_SVR=pd.DataFrame(ypred_sv)
#
...
# line3, = ax.plot(ypred_sv,label='ZI-SVR pred.',linewidth=3)
...
# plt.title('Lightning_prediction_test_comparison_SVR_south_west_area')
# plt.savefig(directory_to_save_figures + 'Lightning prediction test comparison SVR south west area.png', dpi='figure',
...
# plt.close()
#
# prediction_list=[ypred_sv]
# predictor_name=['ZI-SVR']
...
#     plt.title(prdname+' scatter plot test south west area')
...
#     plt.savefig(directory_to_save_figures + prdname+ ' scatter plot test SVR south west area.png', dpi='figure',
#                 format=None, metadata=None,
#                 bbox_inches='tight', pad_inches=0.5,
#                 facecolor='auto', edgecolor='auto',
#                 backend=None, transparent=False)
...
#
# directory_to_save_results="C:/Users/plgeo/OneDrive/PC Desktop/MATLAB DRIVE/MSc_Meteorology/regression results/"
# filetosave3=directory_to_save_results+'SVR_model_southwest_area_test_results.xlsx'
# filetowrite3=pd.ExcelWriter(filetosave3, engine='xlsxwriter')##excel file to save processed data
# df_score=pd.DataFrame(np.array(list_scores_SVR),index=list_score_names,columns=predictor_name)
# df_score.to_excel(filetowrite3,sheet_name='southwest area', engine="xlsxwriter", startrow=1, startcol=0, header=True)
# filetowrite3.save()