...
def evaluate_regressor(model,modelc,eval_set,eval_class_set,cp):
    ...
    for val_round in range(len(Xval_set)):
        ...
        xtrain_nz = xtrain[index_rtrain, :]
        ytrain_nz = ytrain[index_rtrain]
        ...
        ypredval_nz=abs(np.expm1(model.predict(xval_nz)))
        # # yval_fitted=0
...
#GB regression via  bayesian optimisation.

n_trees_gb =hp.quniform('num_boosted_rounds',500,1000,1) #(500,1000) , (400,900) ...
## depth of each tree
max_dep_gb=hp.quniform('max_depth',1,8,1)
# define ratio of features to use for each tree
num_feat_gb=hp.uniform('colsample_bytree',0.1,0.5)
# define ratio of features to use for each node(split).
num_split_gb=0.5
## define subsample fraction for stochastic gradient descent
sub_sample_gb=hp.uniform('subsample',0.700,0.900)
## define shrinkage rates (learning rates)
learn_rate_gb = hp.uniform('learning_rate',0.01,0.021)
...
sum_child_node=hp.quniform('min_child_weight', 1, 5, 1)
## L2-regularisation
reg_lambda=hp.uniform('reg_lambda',0.0001,0.0005)
## maximum delta step
max_delta_step=hp.quniform('max_delta_step',1,5,1)
control_sampling=hp.uniform('control',0.5,1.5)
gbm_grid_params = {'num_boosted_rounds': n_trees_gb,'max_depth': max_dep_gb,'subsample':sub_sample_gb,'learning_rate':learn_rate_gb,'gamma':gamma_gb,'reg_lambda':reg_lambda,'min_child_weight':sum_child_node,'max_delta_step':max_delta_step,'colsample_bytree':num_feat_gb,'control':control_sampling}
...
gbtrial=XGBRegressor(booster='gbtree', grow_policy='lossguide',
                              colsample_bylevel=1, colsample_bynode=num_split_gb, importance_type='gain',
                              validate_parameters=1, verbosity=0, threads=1,random_state=random_seed)
control_par=1.0
...
while n>3:
    ...
    pipeline_trial = Pipeline(steps=[('scaler', StandardScaler()), ('regressor', gbtrial)])
...
corr_factor1 = mean_nz / mean_nzz  ## correction based on long-term probabilities of the thunderstorm /no - thunderstorm in the training sample.
...
cachedir = mkdtemp()
def optfunc_gbregrmodels(gbm_grid_params):
    ...
    gbregr=XGBRegressor(booster='gbtree', grow_policy='lossguide',
                              colsample_bylevel=1, colsample_bynode=num_split_gb, importance_type='gain',
                              validate_parameters=1, verbosity=0, threads=1,random_state=random_seed)
    pipeline_gbregr = Pipeline(steps=[('scaler', StandardScaler()),('regressor',gbregr)],memory=cachedir)
    pipeline_gbregr['regressor'].set_params(**gb_param_search)
    RSQ_eval, RMSE_eval, LogL_eval,excess_variance = evaluate_regressor(pipeline_gbregr,model_class,train_validation_set,train_validation_class_set,gbm_grid_params['control'])
    loss = LogL_eval  ## use mean poisson deviance loss of the 5-fold cross validation as the minimisation function
    return {'loss': loss, 'status': STATUS_OK, 'model_gb': pipeline_gbregr,'Rscore':RSQ_eval,'error':[RMSE_eval,excess_variance]}

trials = Trials()
best_hyperparams_gbreg = fmin(fn=optfunc_gbregrmodels, space=gbm_grid_params, algo=tpe.suggest, max_evals=100,trials=trials,rstate=random_seed)
rmtree(cachedir)
best_GBregrmodel=trials.results[np.argmin([r['loss'] for r in trials.results])]['model_gb'] #get the best model after optimization
best_control_param = best_hyperparams_gbreg['control'] ## optimal hyperparameter controlling sampling weights of the target variable.
...
print("GB log loss on validation: "+str(mean_val_error))
print("GB rmse error on validation: " + str(RMSE_val))  ## prints the mean cross validation RMSE of the best RF model.
print('GB Coef. of determination on validation', RSQ_val)
print('GB excess variance', Excess_var)
...
best_GBregrmodel.fit(train_regr_dataset, np.log1p(ytrain_nzz),regressor__sample_weight=initial_weight)  ## refit the best model with all training data, including validation set.
...
precision, recall, _ = precision_recall_curve(yclass_test, model_class.predict_proba(standardizer_class.transform(df_test_class_dataset.loc[:,:].values))[:,1])
auc_score=auc(recall,precision)

# # ## extrapolation on test data
ypred_gb = np.zeros((y_test.shape[0],))
for inst in range(xclass_test.shape[0]):
    ...
    if ytest_class==0:
         ...
         yprt=abs(np.expm1(best_GBregrmodel.predict(input_rtest)))
         if yprt<0.5:
             yprt=0
         else:
             yprt=yprt*corr_factor4
    ypred_gb[inst] = yprt
log_error_gb= round(mean_squared_log_error(y_test[y_test>0],ypred_gb[np.where(y_test>0)[0].tolist()],squared=False),3)
error_gb = round(mean_squared_error(y_test[y_test>0], ypred_gb[np.where(y_test>0)[0].tolist()], squared=False),3)
print('GB rmse error on test: ' + str(error_gb) + ' ' + str(log_error_gb))
RSQ_test = round(r2_score(y_test[y_test>0],ypred_gb[np.where(y_test>0)[0].tolist()], multioutput='variance_weighted'),3)
RSQ_test_all = round(r2_score(y_test,ypred_gb, multioutput='variance_weighted'),3)
correl_spear,p_test=spearmanr(y_test[y_test>0],ypred_gb[np.where(y_test>0)[0].tolist()],axis=0,nan_policy='omit',alternative='greater')
correl_spear_all,p_test_all=spearmanr(y_test,ypred_gb,axis=0,nan_policy='omit',alternative='greater')
print('Coefficient of determination on test: ' + str(RSQ_test))
print('Spearman correlation on test: ' + str(round(correl_spear,3)))
# ## time-series plot
# fig,ax=plt.subplots(figsize=(12, 8))
# line1, = ax.plot(ypred_gb, label='predicted',linewidth=4)
...
# plt.title('Lightning strikes GB prediction test central area')
# plt.savefig(directory_to_save_figures + 'Lightning strikes GB prediction test central area.png', dpi='figure',
...
# filetosavevars = directory_to_save_models+'ZI_GBR_features.pkl'
# pickle.dump(important_features1,open(filetosavevars,'wb'))
# filetosave1=directory_to_save_models+'GB_ensemble_'+'central area.sav'
# pickle.dump(best_GBregrmodel, open(filetosave1, 'wb'))
# filetosave2=directory_to_save_models+'GB_KDE_'+'central area.sav'
# pickle.dump(model_KDE, open(filetosave2, 'wb'))
#
# stat_predict=describe(ypred_gb)
...
# filetosave3=directory_to_save_results+'GB_regression_central_area_validation_results.xlsx'
...
# df_best_gbr = pd.DataFrame(list(best_hyperparams_gbreg.values()), index=list(best_hyperparams_gbreg.keys()),columns=['value'])
# df_head1 = pd.DataFrame([], columns=["", "", "Bayesian optimized ZI-GBR best hyperparameters", "", ""])
# df_head1.to_excel(filetowrite3, sheet_name='central area', engine="xlsxwriter", startrow=1, startcol=8, header=True)
# df_best_gbr.to_excel(filetowrite3, sheet_name='central area', engine="xlsxwriter", startrow=2, startcol=8, header=True)
# regr_test=pd.DataFrame(np.array([log_error_gb,error_gb,RSQ_test,RSQ_test_all,correl_spear,correl_spear_all]),index=['log loss','RMSE','Rsquared','Rsquared_all','Spearman','Spearman_all'],columns=['test'])
