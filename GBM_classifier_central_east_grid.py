...
from sklearn.metrics import mean_squared_error,r2_score,accuracy_score,confusion_matrix,balanced_accuracy_score,roc_auc_score,classification_report
...
input_data_train0 = input_data_help[0:round(input_data_help.shape[0] * 0.75), :, 8]
input_data_train1 = input_data_help[0:round(input_data_help.shape[0] * 0.75), :, 3]
input_data_train2 = input_data_help[0:round(input_data_help.shape[0] * 0.75), :, 8]
input_data_train3 = input_data_help[0:round(input_data_help.shape[0] * 0.75), :, 11]
lon0=(Lon_intr[0]-float(str(Lon_intr[0])[0]+'0'))*np.ones((round(input_data_help.shape[0] * 0.75),1))
lon1=(Lon_intr[1]-float(str(Lon_intr[1])[0]+'0'))*np.ones((round(input_data_help.shape[0] * 0.75),1))
lon2=(Lon_intr[2]-float(str(Lon_intr[2])[0]+'0'))*np.ones((round(input_data_help.shape[0] * 0.75),1))
lon3=(Lon_intr[3]-float(str(Lon_intr[3])[0]+'0'))*np.ones((round(input_data_help.shape[0] * 0.75),1))
lat0=(Lat_intr[0]-float(str(Lat_intr[0])[0]+'0'))*np.ones((round(input_data_help.shape[0] * 0.75),1))
lat1=(Lat_intr[1]-float(str(Lat_intr[1])[0]+'0'))*np.ones((round(input_data_help.shape[0] * 0.75),1))
lat2=(Lat_intr[2]-float(str(Lat_intr[2])[0]+'0'))*np.ones((round(input_data_help.shape[0] * 0.75),1))
lat3=(Lat_intr[3]-float(str(Lat_intr[3])[0]+'0'))*np.ones((round(input_data_help.shape[0] * 0.75),1))
input_data_train0=np.column_stack((input_data_train0,lat0,lon0))
input_data_train1=np.column_stack((input_data_train1,lat1,lon1))
input_data_train2=np.column_stack((input_data_train2,lat2,lon2))
input_data_train3=np.column_stack((input_data_train3,lat3,lon3))
input_data_train = np.concatenate((input_data_train0, input_data_train1, input_data_train2, input_data_train3))
light_binary_train_help = Strike_grid_categ[0:round(input_data_help.shape[0] * 0.75), index_]
light_binary_train = np.reshape(light_binary_train_help,(light_binary_train_help.shape[0] * 4,1),order='F')
input_data_test0 = input_data_help[round(input_data_help.shape[0] * 0.75):, :,8]
input_data_test1 = input_data_help[round(input_data_help.shape[0] * 0.75):, :, 3]
input_data_test2 = input_data_help[round(input_data_help.shape[0] * 0.75):, :, 8]
input_data_test3 = input_data_help[round(input_data_help.shape[0] * 0.75):, :, 11]
lon0t=(Lon_intr[0]-float(str(Lon_intr[0])[0]+'0'))*np.ones((round(input_data_help.shape[0] * 0.25),1))
lon1t=(Lon_intr[1]-float(str(Lon_intr[1])[0]+'0'))*np.ones((round(input_data_help.shape[0] * 0.25),1))
lon2t=(Lon_intr[2]-float(str(Lon_intr[2])[0]+'0'))*np.ones((round(input_data_help.shape[0] * 0.25),1))
lon3t=(Lon_intr[3]-float(str(Lon_intr[3])[0]+'0'))*np.ones((round(input_data_help.shape[0] * 0.25),1))
lat0t=(Lat_intr[0]-float(str(Lat_intr[0])[0]+'0'))*np.ones((round(input_data_help.shape[0] * 0.25),1))
lat1t=(Lat_intr[1]-float(str(Lat_intr[1])[0]+'0'))*np.ones((round(input_data_help.shape[0] * 0.25),1))
lat2t=(Lat_intr[2]-float(str(Lat_intr[2])[0]+'0'))*np.ones((round(input_data_help.shape[0] * 0.25),1))
lat3t=(Lat_intr[3]-float(str(Lat_intr[3])[0]+'0'))*np.ones((round(input_data_help.shape[0] * 0.25),1))
input_data_test0=np.column_stack((input_data_test0,lat0t,lon0t))
input_data_test1=np.column_stack((input_data_test1,lat1t,lon1t))
input_data_test2=np.column_stack((input_data_test2,lat2t,lon2t))
input_data_test3=np.column_stack((input_data_test3,lat3t,lon3t))
input_data_test = np.concatenate((input_data_test0, input_data_test1, input_data_test2, input_data_test3))
light_binary_test_help = Strike_grid_categ[round(input_data_help.shape[0] * 0.75):, index_]
light_binary_test = np.reshape(light_binary_test_help,(light_binary_test_help.shape[0] * 4,1),order='F')
index_train_help = [list_day_extension(st) for st in range(len(light_binary_train)) if light_binary_train[st] > 0]  ## keeps the indices of the non-zero lightning timestamps and the +-12 hours around them and creates a nested list of timestampd indices around any non-zero element.
...
ycat_train = light_binary_train[index_train]
...
in_data = input_data_train[index_train, :]
...
del index_test_help_help
del index_test_help
io_dataset_class_train = pd.DataFrame(np.column_stack((in_data, ycat_train)))
io_dataset_class_train.columns = inp_features
io_dataset_class_test = pd.DataFrame(np.column_stack((input_data_test, ycat_test)))
io_dataset_class_test.columns = inp_features
xclass_train = io_dataset_class_train.iloc[:, 0:-1].values
yclass_train = io_dataset_class_train.iloc[:, -1].values
...
standardizer.fit(xclass_train)  # standardizing the feature values at mean 0 , std 1
train_class_dataset = standardizer.transform(xclass_train)
df_train_class_dataset = pd.DataFrame(train_class_dataset, columns=feature_array[0:-1])
test_class_dataset = standardizer.transform(xclass_test)
df_test_class_dataset = pd.DataFrame(test_class_dataset, columns=feature_array[0:-1])
del index_train
dict_cv = cross_val_class_set(train_class_dataset, yclass_train)
...
y_sub_train = yclass_train[0:int(round(yclass_train.size * 0.75, 0))]
y_zeros = y_sub_train[y_sub_train == 0]
y_ones = y_sub_train[y_sub_train == 1]
spw = int(y_zeros.size / y_ones.size)  # scale pos weight for GB classifier
...
filetosave_0="C:/Users/plgeo/OneDrive/PC Desktop/MATLAB DRIVE/MSc_Meteorology/GBM_classifier_validation_results_central_east_area_res_12km.xlsx"
filetowrite_0=pd.ExcelWriter(filetosave_0, engine='xlsxwriter')##excel file to save processed data
filetosave_3="C:/Users/plgeo/OneDrive/PC Desktop/MATLAB DRIVE/MSc_Meteorology/GBM_classifier_advanced_scores_central_east_area_res_12km.xlsx"
...
df_head1.to_excel(filetowrite_0, sheet_name='east area', engine="xlsxwriter", startrow=1, startcol=8, header=True)
df_best_gbclass.to_excel(filetowrite_0, sheet_name='south area', engine="xlsxwriter", startrow=2, startcol=8, header=True)
df_gbheader1 = pd.DataFrame([], columns=["", "", "Bayesian optimized GBM classifier validation scores", "", ""])
df_gbheader1.to_excel(filetowrite_0, sheet_name='east area', engine="xlsxwriter", startrow=1,startcol=0, header=True)
class_val_report_gb, list_val_scores_gb = evaluate_classifier(best_gbclassmodel, train_validation_set)
class_val_report_gb.to_excel(filetowrite_0, sheet_name='east area', engine="xlsxwriter", startrow=2,startcol=0, header=True)
...
print('\n')
best_gbclassmodel.fit(df_train_class_dataset, yclass_train)
ypred_gb = best_gbclassmodel.predict(df_test_class_dataset)
tn_gb, fp_gb, fn_gb, tp_gb = confusion_matrix(yclass_test, ypred_gb, normalize='all').ravel()
conf_list_gb = [round(tn_gb, 3), round(fp_gb, 3), round(fn_gb, 3), round(tp_gb, 3)]
class_report_gb, list_scores_gb = test_classifier(yclass_test, ypred_gb)
df_gbheader2 = pd.DataFrame([], columns=["", "", "Bayesian optimized GBM classifier test scores", "", ""])
df_gbheader2.to_excel(filetowrite_0, sheet_name='east area', engine="xlsxwriter", startrow=9, startcol=0, header=True)
class_report_gb.to_excel(filetowrite_0, sheet_name='east area', engine="xlsxwriter", startrow=10,startcol=0, header=True)
print('GB classification accuracy on test:\n', class_report_gb)
print('GB advanced skill scores on test:\n', list_scores_gb)
print('\n')

## load trained GB classifier from central area and apply him also to this area as a comparison
fileclass=directory_to_save_models+'GB_classifier_'+'central grid.sav'
filestandclass= directory_to_save_models+'GB_classifier_standardizer_'+'central grid.sav'
model_class = pickle.load(open(fileclass, 'rb'))
standardizer_class = pickle.load(open(filestandclass,'rb'))
ypred_cgb=model_class.predict(standardizer_class.transform(xclass_test))
class_report_cgb, list_scores_cgb = test_classifier(yclass_test, ypred_cgb)
df_gbheader3 = pd.DataFrame([], columns=["", "", "Bayesian optimized GBM central classifier test scores", "", ""])
df_gbheader3.to_excel(filetowrite_0, sheet_name='east area', engine="xlsxwriter", startrow=17, startcol=0, header=True)
class_report_cgb.to_excel(filetowrite_0, sheet_name='east area', engine="xlsxwriter", startrow=18,startcol=0, header=True)

filetosave1 = directory_to_save_models + 'GB_classifier_' + 'central_area_east.sav'
...
df_validation_scores.to_excel(filetowrite_3, sheet_name='east area', engine="xlsxwriter", startrow=1, startcol=0,header=True)
Test_advanced_scores = list_scores_gb.values
Test_advanced_scoresc=list_scores_cgb.values
...
df_test_scores.to_excel(filetowrite_3, sheet_name='east area', engine="xlsxwriter", startrow=8, startcol=0, header=True)
df_test_scoresc = pd.DataFrame(Test_advanced_scoresc, index=['GBC'], columns=['Heide skill', 'Pierce skill', 'Hit rate', 'False alarm rate', 'Sensitivity','F1 score'])
df_test_scoresc.to_excel(filetowrite_3, sheet_name='east area', engine="xlsxwriter", startrow=12, startcol=0, header=True)
filetowrite_0.save()
filetowrite_3.save()
...
plt.xlabel("GBM classifier central area east feature importance")
plt.savefig(directory_to_save_figures+'XGBoost classifier central area east feature importances.png',dpi='figure', format=None, metadata=None,
