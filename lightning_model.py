...
for i in range(2,121,1):
    ...
    # for res in result_proba:
    #     print(res)

    # for res in result_regressor:
    #     print(res)

    result = np.array(result)
    result = np.row_stack((result[0], result))
    result = np.row_stack((result, result[-1]))
    result = np.column_stack((result[:,0], result))
    result = np.column_stack((result, result[:,-1]))

    result_proba = np.array(result_proba)
    result_proba = np.row_stack((result_proba[0], result_proba))
    result_proba = np.row_stack((result_proba, result_proba[-1]))
    result_proba = np.column_stack((result_proba[:,0], result_proba))
    result_proba = np.column_stack((result_proba, result_proba[:,-1]))

    result_regressor = np.array(result_regressor)
    result_regressor = np.row_stack((result_regressor[0], result_regressor))
    result_regressor = np.row_stack((result_regressor, result_regressor[-1]))
    result_regressor = np.column_stack((result_regressor[:,0], result_regressor))
    result_regressor = np.column_stack((result_regressor, result_regressor[:,-1]))
    # print(result)

    # figure(figsize=(20, 10))
    #
    # xmin, xmax = min(lons)-0.123/2, max(lons)+0.123/2
    # ymin, ymax = min(lats)-0.118/2, max(lats)+0.118/2
    ...
    # ax.add_geometries([poly], crs=ll_proj, facecolor='none', edgecolor='white', linewidth=0.618)
    #
    ...
    # msk = Polygon(rect_from_bound(*exts)).difference( [poly][0].simplify(0.001) )
    # msk_stm  = st_proj.project_geometry (msk, ll_proj)  # project geometry to the projection used by stamen
    #
    # # get and plot Stamen images
    # ax.add_image(stamen_terrain, 10) # this requests image, and plot
    #
    # # plot the mask using semi-transparency (alpha=0.65) on the masked-out portion
    # ax.add_geometries(msk_stm, st_proj, zorder=12, facecolor='white', edgecolor='none', alpha=0.5)
    #
    # ax.gridlines(draw_labels=True, zorder=5)
    # # ax.coastlines(linewidth=1, zorder=10, resolution='10m', color='black')
    # ax.add_feature(cf.NaturalEarthFeature('physical', 'land', '10m', edgecolor='black', facecolor='#FF000000', zorder=5))
    #
    # plt.contourf([min(lons)-0.123/2]+lons+[max(lons)+0.123/2], [max(lats)+0.118/2]+lats[::-1]+[min(lats)-0.118/2], result_regressor, clevs, transform=ll_proj, cmap=cmap, norm=norm, zorder=4)
    # for lat_index in range(len(lats)):
    #     for lon_index in range(len(lons)):
    #         ax.text(x=23.4885+0.123*(lon_index)-0.03, y=38.135-0.118*(lat_index), s=str(round(result_proba[lat_index+1][lon_index+1]*100))+'%', transform = ccrs.PlateCarree(), fontsize=11, color='black', alpha=0.8, zorder=100)
    #
    # cbar = plt.colorbar(mpl.cm.ScalarMappable(cmap=cmap, norm=norm), orientation="vertical", fraction=0.0238, pad=0.12)
    # cbar.set_ticks(clevs[1:-1])
    #
    # date = datetime(1970,1,1)+timedelta(seconds=int(time_list[0]))
    # date_formatted = date.strftime("%d-%m-%Y %H:%M")
    # day = date.strftime("%a")
    #
    # init_date = datetime(1970,1,1)+timedelta(seconds=int(time_list[0]))-timedelta(hours=i)
    # init_date_formatted = init_date.strftime("%d-%m-%Y %H")
    # init_day = init_date.strftime("%a")
    #
    # plt.title("Thunderstorm probability and Lightning density prediction\nvalid for: "+day+" "+date_formatted+" UTC", fontsize=14, y=1.06)
    # text = plt.annotate('Init: GFS '+init_day+' '+init_date_formatted+'z', xy=(0.31, 1.045), color='black', fontsize=10, xycoords='axes fraction')
    # text = plt.annotate('Thunderstorm probability: text annotation (%)\nLightning density: shaded (flashes / (144km$\mathregular{^{2}}$ * hour))', xy=(0.01, -0.09), color='black', fontsize=8, xycoords='axes fraction')
    # # plt.text(s="Plot by Giannis Dravilas", fontsize=10, y=0.005, x=0.02, transform=ax.transAxes)
    # plt.text(s="citizenweather.gr", fontsize=12, y=0.008, x=0.4, transform=ax.transAxes, zorder=6)
    #
    # plot = plt.savefig('/var/www/wordpress/wp-content/uploads/lightning-model/lightning_density_'+str(i).zfill(2)+'.png', bbox_inches='tight')
    #
    # file_name = '/var/www/wordpress/wp-content/uploads/lightning-model/lightning_density_'+str(i).zfill(2)+'.png'
    # picture = Image.open('/var/www/wordpress/wp-content/uploads/lightning-model/lightning_density_'+str(i).zfill(2)+'.png')
    # img = picture.quantize(method=2)
    ...
    # ax.add_feature(cf.NaturalEarthFeature('physical', 'land', '10m', edgecolor='black', facecolor='#FF000000', zorder=5))
    #
    # plt.contourf([min(lons)-0.123/2]+lons+[max(lons)+0.123/2], [max(lats)+0.118/2]+lats[::-1]+[min(lats)-0.118/2], result, transform=ll_proj, cmap=cmap2, norm=norm2, zorder=4)
    # for lat_index in range(len(lats)):
    #     for lon_index in range(len(lons)):
    #         # print(result_regressor[lat_index+1][lon_index+1])
    #         if result_regressor[lat_index+1][lon_index+1] == -0.1:
    #             ax.text(x=23.4885+0.123*(lon_index)-0.03, y=38.135-0.118*(lat_index), s=str(round(result_proba[lat_index+1][lon_index+1], 2)), transform = ccrs.PlateCarree(), fontsize=9, color='black', alpha=0.8, zorder=100)
    #         else:
    #             ax.text(x=23.4885+0.123*(lon_index)-0.03, y=38.135-0.118*(lat_index)-0.02, s=str(round(result_proba[lat_index+1][lon_index+1], 2))+'\n'+str(round(result_regressor[lat_index+1][lon_index+1], 1)), transform = ccrs.PlateCarree(), fontsize=9, color='black', alpha=0.8, zorder=100)
    #
    # cbar = plt.colorbar(mpl.cm.ScalarMappable(cmap=cmap2, norm=norm2), orientation="horizontal", fraction=0.0238, pad=0.04)
    # cbar.set_ticks([0.25,0.75])
    # cbar.ax.set_xticklabels(['No Thunderstorm', 'Thunderstorm'])
    #
    # date = datetime(1970,1,1)+timedelta(seconds=int(time_list[0]))
    # date_formatted = date.strftime("%d-%m-%Y %H:%M")
    # day = date.strftime("%a")
    ...
    # init_day = init_date.strftime("%a")
    #
    # plt.title("Thunderstorm prediction valid for: "+day+" "+date_formatted+" UTC", fontsize=16, y=1.06)
    # text = plt.annotate('Init: GFS '+init_day+' '+init_date_formatted+'z', xy=(0.31, 1.045), color='black', fontsize=10, xycoords='axes fraction')
    # # plt.text(s="Plot by Giannis Dravilas", fontsize=10, y=0.005, x=0.02, transform=ax.transAxes)
    # plt.text(s="citizenweather.gr", fontsize=12, y=0.008, x=0.4, transform=ax.transAxes, zorder=6)
    #
    # plot = plt.savefig('/var/www/wordpress/wp-content/uploads/lightning-model/thunderstorm_classification_'+str(i).zfill(2)+'.png', bbox_inches='tight')
    #
    # file_name = '/var/www/wordpress/wp-content/uploads/lightning-model/thunderstorm_classification_'+str(i).zfill(2)+'.png'
    # picture = Image.open('/var/www/wordpress/wp-content/uploads/lightning-model/thunderstorm_classification_'+str(i).zfill(2)+'.png')
    # img = picture.quantize(method=2)
    # img.save(file_name, optimize=True)
    #
    # plt.clf()
    ...
    # destination = "/var/georgepl/lightning_model/gfs_gribs/gfs_"+str(i).zfill(2)+".grib2"
    # shutil.move("/var/georgepl/lightning_model/gfs.grib2", destination)

    # destination = "/var/georgepl/lightning_model/gfs_gribs/gfs_prev_"+str(i).zfill(2)+".grib2"
    # shutil.move("/var/georgepl/lightning_model/gfs_prev.grib2", destination)
